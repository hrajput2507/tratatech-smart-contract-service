...
async function main() {
  console.log("🚀 Starting TrataTech Smart Contract Deployment on Polygon Amoy Testnet");
  ...
  try {
    ...
    const deploymentFile = `deployment-upgradeable-polygonAmoy-${Date.now()}.json`;
    ...
    console.log(`   MINIMAL_FORWARDER_AMOY=${minimalForwarderAddress}`);
    console.log(`   TRATATECH_FORWARDER_AMOY=${tratatechForwarderAddress}`);
    ...
    const deploymentFile = `deployment-upgradeable-polygonAmoy-failed-${Date.now()}.json`;
    fs.writeFileSync(deploymentFile, JSON.stringify(deploymentResults, null, 2));
